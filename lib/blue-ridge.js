...
if (typeof BlueRidge.Browser === 'undefined') {
  ...
  BlueRidge.Browser = {
    ...
    processRequires: function() {
      if (BlueRidge.Browser.requirements.length === 0) {
        // call document ready (maybe)
    jQuery(function() {alert($.isReady);});
        jQuery.ready();
        return;
      } else {
        var r = BlueRidge.Browser.requirements.shift();
        r.options = r.options || {};
        r.options['onload'] = BlueRidge.Browser.processRequires;
        BlueRidge.Browser.includeRequirement(r.url, r.options);
      }
    },

    includeRequirement: function(url, options) {
      options = options || {};
